...
fn default_action_noop_interval() -> u128 {
    500
}
...
impl Configs {
    ...
    fn validate_user_defined_config(&self) -> bool {
        ...
        validate_field!(
            result,
            self.action_noop_interval,
            default_action_noop_interval()
        );
        validate_field!(
            result,
...
impl Configs {
    ...
    fn merge_config(&mut self, new_config: Configs) {
        ...
        let parent_matches: Vec<Match> = self
            .matches
            .iter()
            .filter(|&m| !match_trigger_set.contains(&m.trigger))
            .cloned()
            .collect();
...
impl ConfigSet {
    pub fn load(config_dir: &Path, package_dir: &Path) -> Result<ConfigSet, ConfigLoadError> {
        ...
        if default.conflict_check {
            for s in specific.iter() {
                ...
                if has_conflicts {
                    eprintln!(
                        "Warning: some triggers had conflicts and may not behave as intended"
                    );
    ...
    fn has_conflicts(default: &Configs, specific: &Vec<Configs>) -> bool {
        let mut sorted_triggers: Vec<String> =
            default.matches.iter().map(|t| t.trigger.clone()).collect();
        ...
        for s in specific.iter() {
            let mut specific_triggers: Vec<String> =
                s.matches.iter().map(|t| t.trigger.clone()).collect();
...
mod tests {
    use super::*;
    use crate::matcher::{MatchContentType, TextContent};
    use std::any::Any;
    use std::io::Write;
    ...
    fn test_config_set_specific_file_duplicate_name() {
        ...
        let user_defined_path2 = create_user_config_file(
            data_dir.path(),
            "specific2.yml",
            r###"
        name: specific1
    ...
    fn test_user_defined_config_set_merge_with_parent_matches() {
        ...
        assert!(config_set.specific[0]
            .matches
            .iter()
            .find(|x| x.trigger == "hello")
            .is_some());
        assert!(config_set.specific[0]
            .matches
            .iter()
            .find(|x| x.trigger == ":lol")
            .is_some());
        assert!(config_set.specific[0]
            .matches
            .iter()
            .find(|x| x.trigger == ":yess")
            .is_some());
    }
    ...
    fn test_config_set_default_parent_works_correctly() {
        ...
        assert!(config_set
            ...
            .matches
            .iter()
            .any(|m| m.trigger == "hasta"));
        assert!(config_set
            .default
            .matches
            .iter()
            .any(|m| m.trigger == "hello"));
    }
    ...
    fn test_config_set_package_configs_default_merge() {
        ...
        let package_path = create_package_file(
            package_dir.path(),
            "package1",
            "package.yml",
            r###"
    ...
    fn test_config_set_package_configs_multiple_files() {
        ...
        let package_path2 = create_package_file(
            package_dir.path(),
            "package1",
            "addon.yml",
            r###"
