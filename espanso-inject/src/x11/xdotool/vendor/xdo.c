...
static int _xdo_send_keysequence_window_to_keycode_list(const xdo_t *xdo,
                                                        const char *keyseq,
                                                        charcodemap_t **keys,
                                                        int *nkeys);
static int _xdo_send_keysequence_window_do(const xdo_t *xdo, Window window,
                                           const char *keyseq, int pressed,
...
static int _xdo_query_keycode_to_modifier(XModifierKeymap *modmap,
                                          KeyCode keycode);
static int _xdo_mousebutton(const xdo_t *xdo, Window window, int button,
                            int is_press);
...
#define DISPLAY_HINT                                                           \
    "Is there an Xorg or other X server running? You can try setting 'export " \
    "DISPLAY=:0' and trying again."
    if (display_name == NULL) {
        fprintf(stderr,
                "Error: No DISPLAY environment variable is set. " DISPLAY_HINT
                "\n");
        return NULL;
    }
    ...
    if (*display_name == '\0') {
        fprintf(stderr,
                "Error: DISPLAY environment variable is empty. " DISPLAY_HINT
                "\n");
        return NULL;
    }
...
xdo_t *xdo_new_with_opened_display(Display *xdpy, const char *display,
    ...
    // Unfortunately, XWayland does not correctly support the features used by
    // libxdo and xdotool.\n", display); return NULL;
    //}
...
int xdo_get_window_location(const xdo_t *xdo, Window wid, int *x_ret,
    ...
    if (ret != 0) {
        ...
        if (parent == attr.root) {
            ...
            XTranslateCoordinates(xdo->xdpy, wid, attr.root, 0, 0, &x, &y,
                                  &unused_child);
        }
...
int xdo_set_window_size(const xdo_t *xdo, Window window, int width, int height,
                        int flags) {
    XWindowChanges wc;
    int ret = 0;
    int cw_flags = 0;
...
int xdo_set_window_property(const xdo_t *xdo, Window wid, const char *property,
    ...
    ret = XChangeProperty(
        xdo->xdpy, wid, XInternAtom(xdo->xdpy, netwm_property, False),
...
int xdo_wait_for_window_size(const xdo_t *xdo, Window window,
    ...
    // printf("Want: %udx%ud\n", width, height);
    if (flags & SIZE_USEHINTS) {
        xdo_translate_window_with_sizehint(xdo, window, width, height, &width,
                                           &height);
    } else {
...
int xdo_get_number_of_desktops(const xdo_t *xdo, long *ndesktops) {
    ...
    data = xdo_get_window_property_by_atom(xdo, root, request, &nitems, &type,
                                           &size);
...
int xdo_get_desktop_for_window(const xdo_t *xdo, Window wid, long *desktop) {
    ...
    data = xdo_get_window_property_by_atom(xdo, wid, request, &nitems, &type,
                                           &size);
...
int xdo_select_window_with_click(const xdo_t *xdo, Window *window_ret) {
    ...
    grab_ret = XGrabPointer(xdo->xdpy, screen->root, False, ButtonReleaseMask,
                            GrabModeSync, GrabModeAsync, screen->root, cursor,
                            CurrentTime);
...
int xdo_move_mouse_relative_to_window(const xdo_t *xdo, Window window, int x,
                                      int y) {
    XWindowAttributes attr;
    Window unused_child;
    int root_x, root_y;
    ...
    XTranslateCoordinates(xdo->xdpy, window, attr.root, x, y, &root_x, &root_y,
                          &unused_child);
    return xdo_move_mouse(xdo, root_x, root_y,
...
int _xdo_mousebutton(const xdo_t *xdo, Window window, int button,
                     int is_press) {
    int ret = 0;
    ...
    if (window == CURRENTWINDOW) {
        ...
        ret = XSendEvent(xdo->xdpy, window, True, ButtonPressMask,
                         (XEvent *)&xbpe);
        XFlush(xdo->xdpy);
        free(active_mod);
...
int xdo_get_mouse_location2(const xdo_t *xdo, int *x_ret, int *y_ret,
    ...
    for (i = 0; i < screencount; i++) {
        ...
        ret =
            XQueryPointer(xdo->xdpy, RootWindowOfScreen(screen), &root, &window,
    ...
    if (window_ret != NULL) {
        ...
        if (window != root && window != 0) {
            ...
            if (findret == XDO_ERROR) {
                ...
                findret = xdo_find_window_client(xdo, window, &client,
                                                 XDO_FIND_CHILDREN);
            }
...
int xdo_enter_text_window(const xdo_t *xdo, Window window, const char *string,
    ...
    while ((len = mbsrtowcs(&key.key, &string, 1, &ps))) {
        ...
        if (key.code == 0 && key.symbol == NoSymbol) {
            fprintf(stderr,
                    "I don't know which key produces '%lc', skipping.\n",
                    key.key);
            continue;
        } else {
...
int _xdo_send_keysequence_window_do(const xdo_t *xdo, Window window,
    ...
    if (_xdo_send_keysequence_window_to_keycode_list(xdo, keyseq, &keys,
                                                     &nkeys) == False) {
    ...
    ret = xdo_send_keysequence_window_list_do(xdo, window, keys, nkeys, pressed,
                                              modifier, delay);
    free(keys);
...
int xdo_send_keysequence_window_down(const xdo_t *xdo, Window window,
    ...
    return _xdo_send_keysequence_window_do(xdo, window, keyseq, True, NULL,
                                           delay);
}

int xdo_send_keysequence_window_up(const xdo_t *xdo, Window window,
                                   const char *keyseq, useconds_t delay) {
    return _xdo_send_keysequence_window_do(xdo, window, keyseq, False, NULL,
                                           delay);
}
...
int xdo_send_keysequence_window(const xdo_t *xdo, Window window,
    ...
    int ret = 0;
    int modifier = 0;
    ret += _xdo_send_keysequence_window_do(xdo, window, keyseq, True, &modifier,
                                           delay / 2);
    ret += _xdo_send_keysequence_window_do(xdo, window, keyseq, False,
                                           &modifier, delay / 2);
    return ret;
}
...
int xdo_find_window_client(const xdo_t *xdo, Window window, Window *window_ret,
    ...
    while (!done) {
        ...
        xdo_get_window_property_by_atom(xdo, window, atom_wmstate, &items, NULL,
                                        NULL);

        if (items == 0) {
            /* This window doesn't have WM_STATE property, keep searching. */
            _xdo_debug(xdo,
                       "window %lu has no WM_STATE property, digging more.",
                       window);
            XQueryTree(xdo->xdpy, window, &dummy, &parent, &children,
                       &nchildren);
            ...
            if (direction == XDO_FIND_PARENTS) {
                ...
                for (i = 0; i < nchildren; i++) {
                    ret = xdo_find_window_client(xdo, children[i], &window,
                                                 direction);
...
static void _xdo_charcodemap_from_char(const xdo_t *xdo, charcodemap_t *key) {
    ...
    /* If the character is an uppercase character within the Basic Latin or
     * Latin-1 code block, then sending the capital character keycode will not
     * work. We have to also send the shift modifier. There are only three
     * ranges of capital letters to worry about */
    if ((key->key >= 0x41 && key->key <= 0x5A) ||
        (key->key >= 0xC0 && key->key <= 0xD6) ||
        (key->key >= 0xD8 && key->key <= 0xDE)) {
...
static int _xdo_has_xtest(const xdo_t *xdo) {
    int dummy;
    return (XTestQueryExtension(xdo->xdpy, &dummy, &dummy, &dummy, &dummy) ==
            True);
}
...
static void _xdo_populate_charcode_map(xdo_t *xdo) {
    ...
    KeySym *keysyms = XGetKeyboardMapping(
        xdo->xdpy, xdo->keycode_low, xdo->keycode_high - xdo->keycode_low + 1,
...
int _xdo_send_keysequence_window_to_keycode_list(const xdo_t *xdo,
                                                 const char *keyseq,
                                                 charcodemap_t **keys,
                                                 int *nkeys) {
    char *tokctx = NULL;
...
int xdo_get_window_property(const xdo_t *xdo, Window window,
                            const char *property, unsigned char **value,
                            long *nitems, Atom *type, int *size) {
    *value = xdo_get_window_property_by_atom(
        xdo, window, XInternAtom(xdo->xdpy, property, False), nitems, type,
        size);
    if (*value == NULL) {
        return XDO_ERROR;
    }
...
int _xdo_ewmh_is_supported(const xdo_t *xdo, const char *feature) {
    ...
    results = (Atom *)xdo_get_window_property_by_atom(xdo, root, request,
                                                      &nitems, &type, &size);
    for (i = 0L; i < nitems; i++) {
                ...
                keycode =
                    modifiers
                        ->modifiermap[mod_index * modifiers->max_keypermod +
                                      mod_key];
                if (keycode) {
                    XTestFakeKeyEvent(xdo->xdpy, keycode, is_press,
                                      CurrentTime);
                    XSync(xdo->xdpy, False);
                    break;
                }
...
int xdo_clear_active_modifiers(const xdo_t *xdo, Window window,
    ...
    xdo_send_keysequence_window_list_do(xdo, window, active_mods, active_mods_n,
    ...
    if (!ret && input_state & LockMask) {
        ...
        ret = xdo_send_keysequence_window_down(xdo, window, "Caps_Lock",
                                               DEFAULT_DELAY);
        ret += xdo_send_keysequence_window_up(xdo, window, "Caps_Lock",
                                              DEFAULT_DELAY);
    }
...
int xdo_get_pid_window(const xdo_t *xdo, Window window) {
    ...
    data = xdo_get_window_property_by_atom(xdo, window, atom_NET_WM_PID,
                                           &nitems, &type, &size);
...
int xdo_quit_window(const xdo_t *xdo, Window window) {
    ...
    xev.xclient.message_type =
        XInternAtom(xdo->xdpy, "_NET_CLOSE_WINDOW", False);
    xev.xclient.format = 32;
...
int xdo_get_window_name(const xdo_t *xdo, Window window,
    ...
    *name_ret = xdo_get_window_property_by_atom(xdo, window, atom_NET_WM_NAME,
                                                &nitems, &type, &size);
    if (nitems == 0) {
        *name_ret = xdo_get_window_property_by_atom(xdo, window, atom_WM_NAME,
                                                    &nitems, &type, &size);
    }
    *name_len_ret = nitems;
    *name_type = type;
...
int xdo_get_window_classname(const xdo_t *xdo, Window window,
                             unsigned char **class_ret) {
...
int xdo_window_state(xdo_t *xdo, Window window, unsigned long action,
                     const char *property) {
    int ret;
    XEvent xev;
    Window root = RootWindow(xdo->xdpy, 0);
...
int fast_send_keysequence_window_list_do(const xdo_t *xdo, Window window,
                                         charcodemap_t *keys, int nkeys,
                                         int pressed, int *modifier,
                                         useconds_t delay) {
    int i = 0;
...
void fast_charcodemap_from_keysym(const xdo_t *xdo, charcodemap_t *key,
                                  KeySym keysym) {
    int i = 0;
    int len = xdo->charcodes_len;
...
/* XXX: Return proper code if errors found */
int fast_enter_text_window(const xdo_t *xdo, Window window, const char *string,
                           useconds_t delay) {
    ...
    while ((len = mbsrtowcs(&key.key, &string, 1, &ps))) {
        ...
        //_xdo_send_key(xdo, window, keycode, modstate, True, delay);
        //_xdo_send_key(xdo, window, keycode, modstate, False, delay);
        fast_send_keysequence_window_list_do(xdo, window, &key, 1, True, NULL,
                                             delay / 2);
        key.needs_binding = 0;
        fast_send_keysequence_window_list_do(xdo, window, &key, 1, False, NULL,
                                             delay / 2);
...
void fast_send_event(const xdo_t *xdo, Window window, int keycode,
                     int pressed) {
    XKeyEvent xk;
    xk.display = xdo->xdpy;
    xk.window = window;
...
int fast_send_keysequence_window_do(const xdo_t *xdo, Window window,
                                    const char *keyseq, int pressed,
                                    int *modifier, useconds_t delay) {
    int ret = 0;
    charcodemap_t *keys = NULL;
    ...
    ret = fast_send_keysequence_window_list_do(xdo, window, keys, nkeys,
                                               pressed, modifier, delay);
    free(keys);
...
int fast_send_keysequence_window(const xdo_t *xdo, Window window,
    ...
    int ret = 0;
    int modifier = 0;
    ret += fast_send_keysequence_window_do(xdo, window, keyseq, True, &modifier,
                                           delay / 2);
    ret += fast_send_keysequence_window_do(xdo, window, keyseq, False,
                                           &modifier, delay / 2);
    return ret;
}