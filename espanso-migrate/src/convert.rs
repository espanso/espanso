...
pub fn convert(input_files: HashMap<String, Hash>) -> HashMap<String, ConvertedFile> {
    ...
    for input_path in sorted_input_files {
        ...
        let match_file_path_if_unlisted = if should_generate_match {
            let should_underscore =
                !input_path.starts_with("default") && yaml_parent != Some("default");
        ...
        if should_generate_config {
            ...
            map_field_if_present(
                ...
                "paste_shortcut",
                |val| match val {
                    Yaml::String(shortcut) if shortcut == "CtrlV" => {
                        Some(Yaml::String("CTRL+V".to_string()))
                    }
                    ...
                    Yaml::String(shortcut) if shortcut == "MetaV" => {
                        Some(Yaml::String("META+V".to_string()))
                    }
                    Yaml::String(_) => None,
                    _ => None,
                },
...
fn yaml_get_vec<'a>(yaml: &'a Hash, name: &str) -> Option<&'a Vec<Yaml>> {
    yaml.get(&Yaml::String(name.to_string()))
        .and_then(|v| v.as_vec())
}
...
fn apply_form_syntax_patch(matches: &mut [Yaml]) {
    for m in matches.iter_mut() {
        if let Yaml::Hash(fields) = m {
            if let Some(Yaml::String(form_option)) =
                fields.get_mut(&Yaml::String("form".to_string()))
            {
