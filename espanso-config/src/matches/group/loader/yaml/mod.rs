...
fn apply_prefix(
    ...
    if mode == "agnostic" {
        ...
        if !prefix.is_empty()
            || cleaned.chars().next().map_or(false, |c| {
...
fn apply_suffix(
    ...
    if mode == "agnostic" {
        ...
        if !suffix.is_empty()
            || cleaned.chars().next_back().map_or(false, |c| {
                smart_chars.iter().any(|s| s.chars().next() == Some(c))
            })
        {
...
fn remove_leading_smart_chars(
    ...
    let is_smart_char = smart_chars
        .iter()
        .any(|s| s.chars().next() == Some(first_char));
...
fn remove_trailing_smart_chars(
    ...
    let is_smart_char = smart_chars
        .iter()
        .any(|s| s.chars().next() == Some(last_char));
